...
function buildDocData(doc, extraData) {
    ...
    var excludePath = '';
    doc.stateName = 'home.' + doc.name;
    doc.outputPath = doc.outputPath.replace('partials/api/', '');
    doc.path = doc.path.replace('api/', '');
    doc.path = doc.path.replace('directive/', '');
...
module.exports = function componentRouteProcessor(moduleMap) {
    ...
    function process(docs) {
        ...
        docs.push({
            ...
            template: 'route.template.js',
            outputPath: 'src/component-route-data.js',
